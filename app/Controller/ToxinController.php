	...
	*/
	function index() {
		...
		if ($this->Toxin->getAffectedRows() === 0) {
			throw new NotFoundException('Toxin not found!');
		}

		$this->set('toxin', $toxin);
	}
	
	/**
	 * searches for all toxin by name and returns a json array
	 ...
	 * http://localhost:8000/toxin/search?name=Acute%20tubular%20necrosis
	 * http://localhost:8000/toxin/search?name=Acute%20tubular%20necrosis&type=equals
	...
	function search() {
		...
		// throw exception if we can't find anything.
		if ($this->Toxin->getAffectedRows () === 0) {
			throw new NotFoundException ( 'No toxin found!' );
		}
